...
export default function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  if (session) {
    return <SessionProvider value={{ session, signOut: logout }}>{children}</SessionProvider>;
  }
      ...
      <header className="px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="h-9 w-9 rounded-xl bg-gradient-to-br from-blue-600 to-indigo-600 grid place-items-center text-white shadow-md">
            ...
            >
              <path d="M12 2 5 6v6c0 5 3.5 8.5 7 10 3.5-1.5 7-5 7-10V6l-7-4Z" />
    ...
    <div className="px-8 py-9 md:px-12 md:py-12">
      ...
      <form onSubmit={submit} className="max-w-sm mx-auto space-y-4">
        ...
        <div className="text-[11px] text-slate-500 bg-blue-50/60 border border-blue-100 rounded-lg px-3 py-2 leading-relaxed">
          <span className="font-semibold text-blue-700">Demo mode:</span> any email & password
          works. Create an account to personalize your dashboard with your store.
        </div>
        ...
        {!sent ? (
          <>
            <p className="text-xs text-slate-500 mb-4">
              Enter your email and we'll send a reset link (simulated in this demo).
            </p>
            <input
              type="email"
              value={email}
  ...
  function next() {
    ...
    if (tab === "essentials") {
      if (!form.fullName || !form.email || !form.password)
        return setErr("Please fill in name, email and password.");
  ...
  return (
    <div className="px-6 py-8 md:px-10 md:py-10 bg-gradient-to-b from-white to-slate-50/50">
      <div className="text-center mb-5">
        <div className="text-xs font-bold text-blue-600 tracking-[0.18em] mb-1">
          CREATE YOUR ACCOUNT
        </div>
        <h1 className="text-2xl font-bold text-slate-900 tracking-tight">
          Set up your Freshness Passport
        </h1>
      </div>
        ...
        {tab === "essentials" && (
          <div className="flex flex-col md:flex-row gap-8 items-start">
            <div className="mx-auto md:mx-0 flex-shrink-0">
              ...
              <div className="text-[10.5px] text-slate-500 text-center mt-2 max-w-[8rem]">
                Click pencil to upload
              </div>
            </div>
            ...
            <label className="flex items-start gap-3 p-4 bg-white rounded-xl border border-slate-200 cursor-pointer hover:border-blue-300 transition-colors">
              ...
              <div>
                <div className="text-sm font-semibold text-slate-800">
                  Email me freshness alerts
                </div>
                <div className="text-xs text-slate-500 mt-0.5">
                  Get notified when batches drop below 30% RSL or cold-chain excursions occur.
                </div>
              </div>
            </label>
            <div className="text-[11px] text-slate-500 bg-blue-50/50 border border-blue-100 rounded-lg p-3">
              By creating an account you agree to our demo Terms of Service & Privacy Policy. No
  ...
  return (
    <div className="grid md:grid-cols-2">
      <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-900 text-white p-8 md:p-10 relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <p className="text-sm text-slate-300 leading-relaxed mb-6">
            Manage stores, users, role assignments and platform-wide settings for the Freshness
            Passport network.
          </p>
      ...
      <form onSubmit={submit} className="p-8 md:p-10 space-y-4">
        <div>
          <div className="text-xs font-bold text-indigo-600 tracking-[0.18em] mb-1">
            ADMIN SIGN IN
          </div>
