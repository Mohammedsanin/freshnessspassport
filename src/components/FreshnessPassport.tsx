...
const SEED_STORES: Store[] = [
  {
    ...
    activeSince: new Date("2024-02-10").getTime(),
    tier: "Enterprise",
    type: "Distribution Centre",
    categories: ["Produce", "Dairy", "Bakery", "Meat & Fish", "Ready Meals"],
    rslThreshold: 3,
  },
  {
    id: "S1",
    ...
    activeSince: new Date("2024-04-01").getTime(),
    tier: "Standard",
    type: "Supermarket",
  ...
  },
  {
    ...
    activeSince: new Date("2024-05-15").getTime(),
    tier: "Standard",
    type: "Supermarket",
  ...
  },
  {
    ...
    activeSince: new Date("2024-07-09").getTime(),
    tier: "Pilot",
    type: "Supermarket",
  ...
  },
  {
    ...
    activeSince: new Date("2024-09-22").getTime(),
    tier: "Enterprise",
    type: "Flagship",
    categories: ["Produce", "Bakery", "Ready Meals"],
    rslThreshold: 3,
  },
  {
    id: "S5",
    ...
    activeSince: new Date("2025-01-12").getTime(),
    tier: "Standard",
    type: "Convenience",
...
const TIPS: Record<string, string> = {
  overview:
    "The Overview Dashboard pulls together freshness, waste, and operational KPIs across your whole network.",
  passports:
    "A Freshness Digital Passport is assigned to every product batch — recording movement, temperature exposure, handling and dwell time.",
  ...
  waste:
    "The Waste Root-Cause Analytics engine traces each waste instance back to its operational driver and ranks drivers by impact.",
  action:
    "The Operational Action Engine converts freshness intelligence into specific, executable tasks for your store and DC teams.",
  impact:
    "The Impact & Sustainability Report quantifies the real-world results of the platform's interventions in cost, waste and CO₂.",
  storeProfile:
  ...
  switch (a.type) {
    ...
    case "ADD_STORE": {
      ...
      return {
        ...
        toasts: [
          { id: uid(), kind: "success", text: `Store "${a.store.name}" created` },
          ...s.toasts,
        ],
      };
    }
    case "ADD_BATCH": {
      ...
      return {
        ...
        toasts: [
          { id: uid(), kind: "success", text: `Passport ${a.batch.id} created` },
          ...s.toasts,
        ],
      };
    }
...
const getWasteThisWeek = (s: State, storeId: string) => {
  ...
  return WASTE_INCIDENTS.filter(
    (w) => (storeId === "all" || w.storeId === storeId) && w.date >= cutoff,
...
const getOverdueTaskIds = (s: State) => {
  ...
  return new Set(
    s.tasks.filter((t) => t.status !== "done" && t.assignedAt < cutoff).map((t) => t.id),
  );
};

type Notification = {
  id: string;
  kind: "expiry" | "overdue" | "temp";
  text: string;
  ts: number;
  batchId?: string;
  taskId?: string;
    ...
    <div className="mb-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {tipKey && (
            <button
              onClick={() => dispatch({ type: "TOGGLE_TIP", key: tipKey })}
      ...
      {isOpen && tipKey && TIPS[tipKey] && (
        <div
          ...
          style={{ background: C.light, borderLeft: `3px solid ${C.accent}`, color: C.primary }}
        >
          {TIPS[tipKey]}
        </div>
      )}
...
const KpiCard = ({
  ...
  >
    <button onClick={onClick} className="w-full text-left" disabled={!onClick}>
      ...
      <div className="text-[28px] font-bold mt-1" style={{ color: C.text }}>
        {value}
      </div>
      {sub && (
        <div className="text-[12px] mt-1" style={{ color: C.text2 }}>
          {sub}
        </div>
      )}
    </button>
    ...
    <div className="flex flex-col items-center">
      ...
      <div className="-mt-[120px] flex flex-col items-center" style={{ width: size }}>
        <div className="text-[28px] font-bold" style={{ color: C.text }}>
          {Math.round(score)}
          <span className="text-[14px] font-medium" style={{ color: C.muted }}>
            /100
          </span>
        </div>
      </div>
      <div className="mt-[78px] text-[12px]" style={{ color: C.muted }}>
        Freshness Score
      </div>
    </div>
  );
  ...
  >
    ...
    >
      <div className="flex items-center justify-between p-6 pb-4">
        <h2 className="text-[13px] font-bold uppercase tracking-wider" style={{ color: C.primary }}>
          {title}
        </h2>
        <button
          onClick={onClose}
...
const Field = ({
  ...
  <div className="mb-4">
    ...
    {children}
    {error && (
      <div className="text-[12px] mt-1" style={{ color: C.red }}>
        {error}
      </div>
    )}
  </div>
    ...
    >
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1">
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="Store ID" required error={errors.code}>
                <input
                  className={inputCls}
              ...
              <Field label="Manager" required error={errors.manager}>
                <input
                  className={inputCls}
      ...
      {tab === "ops" && (
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field label="Cold Storage Units">
            <input
              type="number"
              className={inputCls}
              style={inputStyle()}
          ...
          <Field label="WMS">
            ...
            >
              ...
              <option>Blue Yonder</option>
              <option>Korber</option>
            </select>
          </Field>
        ...
        <div>
          {[
            ...
            >
              ...
              <button
                onClick={() => upd(k, !(form as any)[k])}
                className="w-11 h-6 rounded-full transition"
                style={{ background: (form as any)[k] ? C.accent : C.border }}
              >
                <span
                  className="block w-5 h-5 bg-white rounded-full shadow transition"
                  style={{ transform: (form as any)[k] ? "translateX(22px)" : "translateX(2px)" }}
                />
              </button>
            </div>
          ))}
          <Field label="RSL Alert Threshold (days)">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[16px] font-semibold w-8 text-center" style={{ color: C.text }}>
                {form.rslThreshold}
              </span>
              <button
...
function PassportModal({ state, dispatch }: { state: State; dispatch: Dispatch<Action> }) {
  ...
  const [form, setForm] = useState({
    ...
    assignedStaff: "",
    entryPoint: new Date().toISOString().slice(0, 10),
    actionType: "Monitor",
  ...
  const [extractionConfidence, setExtractionConfidence] = useState<"high" | "medium" | "low">(
    "medium",
  );
  ...
  const runExtraction = async () => {
    ...
    try {
      ...
      const nonNullCount = single
        ? Object.values(single).filter((v) => v !== null && v !== "" && v !== undefined).length
        : 0;
      const conf =
        (single?.confidence as any) ||
        (nonNullCount > 6 ? "high" : nonNullCount > 3 ? "medium" : "low");
  ...
  const fieldClass = (key: string) =>
    `${inputCls} ${flashedFields.has(key) ? "fp-field-fill" : ""}`;
  ...
  const create = (overrides?: Partial<typeof form> & { sourceDoc?: Batch["sourceDocument"] }) => {
    ...
    const storeId =
      cur.location === "dc" ? "DC" : cur.location === "transit" ? "DC" : cur.storeSelect;
  ...
  const createAllFromBill = async () => {
    ...
    extractedList.forEach((ex, i) => {
      ...
      const bsl =
        typeof ex.baseShelfLife === "number" ? ex.baseShelfLife : CATEGORY_SHELF[cat] || 7;
      const storeId =
        form.location === "dc" ? "DC" : form.location === "transit" ? "DC" : form.storeSelect;
      ...
      const batch: Batch = {
        ...
        productionDate: new Date(
          parseScannedDate(ex.productionDate) || new Date().toISOString().slice(0, 10),
        ).getTime(),
        printedExpiry: new Date(
          parseScannedDate(ex.printedExpiry) ||
            new Date(Date.now() + bsl * DAY).toISOString().slice(0, 10),
        ).getTime(),
  ...
  const confidenceBadge = () => {
    if (extractionConfidence === "high")
      return (
        <Pill bg="#DCFCE7" color={C.green}>
          High Confidence
        </Pill>
      );
    if (extractionConfidence === "medium")
      return (
        <Pill bg="#FEF3C7" color={C.amber}>
          Review Fields
        </Pill>
      );
    return (
      <Pill bg="#FEE2E2" color={C.red}>
        Manual Check Needed
      </Pill>
    );
  };
        ...
        banner={
          <>
            ...
            <div
              ...
              style={{ background: C.light, border: `1px dashed #93C5FD` }}
            >
              {isDragOver && (
                <div
                ...
                <div className="flex items-center justify-between gap-4 flex-wrap">
                  <div>
                    <div className="text-[13px] font-semibold" style={{ color: C.text }}>
                      📄 Have a delivery bill or invoice?
                    </div>
                    <div className="text-[12px] mt-0.5" style={{ color: C.text2 }}>
                      Upload it and we'll fill the form for you
                    </div>
                ...
                <div>
                  ...
                  {scanFile.type === "application/pdf" ? (
                    <embed
                      ...
                      style={{ height: 200, border: `1px solid ${C.border}`, background: "#fff" }}
                    />
                  ) : (
                    <img
                      src={scanDataUrl}
              ...
              {scanState === "loading" && (
                <div className="flex items-center gap-3 py-2">
                  <div
                    ...
                    style={{ border: `2px solid ${C.accent}`, borderTopColor: "transparent" }}
                  />
                  <div
              ...
              {scanState === "error" && (
                ...
                >
                  <div className="text-[13px] font-semibold mb-2" style={{ color: C.red }}>
                    {scanError || "Could not read the document."}
                  </div>
                  <div className="flex gap-2">
                    {scanFile && (
                      <button
              ...
              {scanState === "nodata" && (
                ...
                >
                  <div className="text-[13px] font-medium mb-2" style={{ color: C.amber }}>
                    We couldn't extract product data from this document. This may not be a delivery
                    note or invoice. Please enter details manually.
                  </div>
                  <button
                    onClick={clearScan}
                    className="px-3 h-8 rounded-lg text-[12px] font-semibold"
                    style={{ border: `1px solid ${C.border}`, color: C.text2, background: "#fff" }}
                  >
                    Enter Manually
                  </button>
                </div>
              ...
              {scanState === "results" && extracted && (
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <div className="text-[13px] font-semibold" style={{ color: C.green }}>
                  ...
                  >
                    {(
                      [
                        ...
                        [
                          "Base Shelf Life",
                          extracted.baseShelfLife ? `${extracted.baseShelfLife} days` : null,
                        ],
                        [
                          "Storage Temp",
                          typeof extracted.storageTemp === "number"
                            ? `${extracted.storageTemp}°C`
                            : null,
                        ],
              ...
              {scanState === "multi" && extractedList && (
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <div>
                      <div className="text-[13px] font-semibold" style={{ color: C.text }}>
                        Multiple products found on this bill
                      </div>
                      <div className="text-[12px]" style={{ color: C.text2 }}>
                        Select which product to create a passport for:
                      </div>
                    </div>
                    {confidenceBadge()}
                  </div>
                      ...
                      >
                        <div className="min-w-0">
                          ...
                          <div className="text-[11px]" style={{ color: C.text2 }}>
                            {p.units ?? "?"} units · Expiry {p.printedExpiry || "—"}
                          </div>
                        </div>
                        <button
            ...
            {successMsg && (
              <div
                className="mb-3 p-3 rounded-lg text-[13px]"
                style={{ background: "#F0FDF4", borderLeft: `3px solid ${C.green}`, color: C.text }}
              >
                {successMsg}
              </div>
            )}
      ...
      >
          ...
          <div className="flex gap-6">
            ...
            <div className="flex-1 grid grid-cols-2 gap-3">
              ...
              >
                ...
                {extractedInvoiceNo && (
                  <div className="text-[11px] mt-1" style={{ color: C.muted }}>
                    Invoice Ref: {extractedInvoiceNo}
                  </div>
                )}
              </Field>
        ...
        {tab === "cold" && (
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Max Allowed Temp (°C)">
              <input
                type="number"
                className={inputCls}
                style={inputStyle()}
            ...
            <Field
              ...
              hint={
                ...
                >
                  {tempInRange ? "Within Range" : "⚠ Excursion"}
                </Pill>
              }
            >
              <input
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Recommended Action">
              ...
              >
                ...
                <option>Allocation Adjust</option>
              </select>
            </Field>
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: "rgba(0,0,0,0.4)", opacity: open ? 1 : 0 }}
        ...
        <div className="p-6">
          <div className="flex items-start justify-between mb-4">
            <div>
              ...
              <h2 className="text-[20px] font-bold mt-1" style={{ color: C.text }}>
                {batch.product}
              </h2>
              <div className="text-[13px] mt-1" style={{ color: C.text2 }}>
                {batch.id} · {batch.sku} · {store?.name}
              </div>
            </div>
            <button
              onClick={() => dispatch({ type: "CLOSE_DRAWER" })}
          ...
          <div className="grid grid-cols-3 gap-3 mb-5">
            <div className="p-3 rounded-lg" style={{ background: C.bg }}>
              ...
              <div className="text-[18px] font-bold" style={{ color: freshnessColor(r.score) }}>
                {r.rslDays}d
              </div>
            </div>
            <div className="p-3 rounded-lg" style={{ background: C.bg }}>
            ...
            <div className="p-3 rounded-lg" style={{ background: C.bg }}>
              ...
              <div className="text-[18px] font-bold" style={{ color: C.text }}>
                {batch.units}
              </div>
            </div>
          </div>

          <h4 className="text-[13px] font-semibold mb-2" style={{ color: C.text }}>
            Temperature History
          </h4>
          ...
          <h4 className="text-[13px] font-semibold mt-5 mb-2" style={{ color: C.text }}>
            RSL Breakdown
          </h4>
          <div className="space-y-1 text-[13px]" style={{ color: C.text2 }}>
            <div>
              Base shelf life: <b>{batch.baseShelfLife}d</b>
            </div>
            <div>
              Days since production: <b>−{r.daysSinceProduction}</b>
            </div>
            <div>
              Temp penalty: <b>−{r.tempPenalty}</b>
            </div>
            <div>
              Dwell penalty: <b>−{r.dwellPenalty}</b>
            </div>
            <div className="pt-2 mt-2 border-t" style={{ borderColor: C.border }}>
              Remaining: <b style={{ color: freshnessColor(r.score) }}>{r.rslDays}d</b>
            </div>
          </div>
            ...
            <div className="mt-5">
              {existingTask ? (
                <div
                  ...
                  style={{ background: "#DCFCE7", borderLeft: `3px solid ${C.green}` }}
                >
                  <span className="text-[13px]" style={{ color: C.green }}>
                    Action task already created ✓
                  </span>
                  <button
                    onClick={() => {
                      dispatch({ type: "CLOSE_DRAWER" });
                      dispatch({ type: "NAVIGATE", page: "actions" });
                      dispatch({ type: "HIGHLIGHT_TASK", id: existingTask.id });
                    }}
                    className="text-[12px] font-semibold"
                    style={{ color: C.green }}
                  >
                    View in Action Engine →
                  </button>
                </div>
              ) : (
                <div
                  className="p-3 rounded-lg"
                  style={{ background: C.light, borderLeft: `3px solid ${C.accent}` }}
                >
                  <div className="text-[13px] mb-2" style={{ color: C.primary }}>
                    Recommended: {rslToAction(r.rslDays)}
                  </div>
                  <button
                    onClick={createTask}
          ...
          {batch.sourceDocument ? (
            ...
            <div className="text-[13px] italic" style={{ color: C.muted }}>
              Manually entered
            </div>
          )}
        </div>
...
function SourceDocSection({ doc }: { doc: NonNullable<Batch["sourceDocument"]> }) {
  ...
  const confColor =
    doc.confidence === "high" ? C.green : doc.confidence === "medium" ? C.amber : C.red;
  const confBg =
    doc.confidence === "high" ? "#DCFCE7" : doc.confidence === "medium" ? "#FEF3C7" : "#FEE2E2";
  return (
    <>
      <div
      ...
      >
        <div className="min-w-0">
          ...
          <div className="text-[11px]" style={{ color: C.muted }}>
            Scanned on {formatDate(doc.scannedAt)}
          </div>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          <Pill bg={confBg} color={confColor}>
            {doc.confidence}
          </Pill>
          <button
            ...
            <div className="p-4">
              {doc.mediaType === "application/pdf" ? (
                <embed
                  ...
                  type="application/pdf"
                  className="w-full rounded-lg"
                  style={{ height: "70vh", border: `1px solid ${C.border}` }}
                />
              ) : (
                <img
                  src={doc.dataUrl}
...
function Overview({ state, dispatch }: PageProps) {
  ...
  const byStore = state.stores
    .filter((s) => s.id !== "DC")
    .map((s) => {
      const sBatches = state.batches.filter((b) => b.storeId === s.id);
      const score = sBatches.length
        ? sBatches.reduce((a, b) => a + computeRSL(b, n).score, 0) / sBatches.length
        : 0;
  ...
  const wasteDrivers = useMemo(() => {
    ...
    WASTE_INCIDENTS.filter((w) => storeId === "all" || w.storeId === storeId).forEach((w) =>
      m.set(w.driver, (m.get(w.driver) || 0) + w.cost),
    );
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-[24px] font-bold" style={{ color: C.text }}>
          Overview
        </h1>
        <p className="text-[13px]" style={{ color: C.text2 }}>
          {storeId === "all" ? "All stores" : state.stores.find((s) => s.id === storeId)?.name}
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KpiCard
          ...
          accent={C.accent}
          onClick={() => dispatch({ type: "NAVIGATE", page: "passports" })}
        />
        <KpiCard
          label="At Risk Products"
          value={atRisk}
          accent={C.red}
          onClick={() => dispatch({ type: "NAVIGATE", page: "rsl" })}
        />
        <KpiCard
          label="Waste This Week"
          value={formatGBP(waste)}
          accent={C.amber}
          onClick={() => dispatch({ type: "NAVIGATE", page: "analytics" })}
        />
              ...
              <Bar dataKey="score">
                {byStore.map((b) => (
                  <Cell
                    ...
                    fillOpacity={storeId === "all" || storeId === b.id ? 1 : 0.3}
                  />
                ))}
              </Bar>
            </BarChart>
          ...
          <ResponsiveContainer width="100%" height={240}>
            <PieChart>
              <Pie
                ...
                outerRadius={90}
              >
                {wasteDrivers.map((_, i) => (
                  <Cell key={i} fill={DCOLORS[i % DCOLORS.length]} />
                ))}
              </Pie>
      ...
      <Card className="p-5">
        ...
        <div className="space-y-2">
          {feed.length === 0 && (
            <div className="text-[13px] py-4 text-center" style={{ color: C.muted }}>
              No activity yet
            </div>
          )}
          {feed.map((a) => {
            ...
            const click = a.batchId
              ? () => dispatch({ type: "OPEN_DRAWER", batchId: a.batchId! })
              : undefined;
            return (
              <div
                key={a.id}
              ...
              >
                ...
                {store && (
                  <Pill bg={C.badgeBlueBg} color={C.badgeBlueText}>
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[13px]" style={{ color: C.text2 }}>
            Live freshness intelligence for every active batch
          </p>
        </div>
        <Btn
          kind="primary"
      ...
      <Card className="p-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-3">
          ...
          <select
            ...
            style={inputStyle()}
            value={storeId}
            onChange={(e) => dispatch({ type: "SET_STORE_FILTER", storeId: e.target.value })}
          >
        ...
        <div className="text-[12px] mb-3" style={{ color: C.muted }}>
          Showing {filtered.length} of {state.batches.length} passports
        </div>

        {filtered.length === 0 ? (
          <div className="py-12 text-center">
            <div className="text-[48px]" style={{ color: C.muted }}>
              📦
            </div>
            <div className="text-[14px] font-semibold mt-2" style={{ color: C.text }}>
            ...
            <button
              onClick={() => {
                ...
                setStatus("all");
                setCategory("all");
                dispatch({ type: "SET_STORE_FILTER", storeId: "all" });
              }}
          ...
          <div className="overflow-auto">
            <table className="w-full text-[13px]">
              <thead>
                <tr style={{ color: C.muted, fontSize: 11 }} className="text-left">
              ...
              <tbody>
                {filtered.map((b) => {
                  ...
                  return (
                    <tr
                      key={b.id}
                      className={`border-t ${flash ? "fp-flash-row" : ""}`}
                      style={{ borderColor: C.border }}
                    >
                      ...
                      <td style={{ color: freshnessColor(r.score), fontWeight: 600 }}>
                        {r.rslDays}d
                      </td>
                      <td>
                        <StatusBadge score={r.score} />
                      </td>
                      <td>
                        <button
                          onClick={() => dispatch({ type: "OPEN_DRAWER", batchId: b.id })}
                          className="text-[12px] font-semibold"
                          style={{ color: C.accent }}
                        >
                          View →
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-[13px]" style={{ color: C.text2 }}>
          Remaining shelf life across the network
        </p>
      </div>
              ...
              {batches.map(({ b, r }) => {
                ...
                return (
                  <tr
                    key={b.id}
                    className="border-t"
                    style={{ borderColor: C.border, borderLeft: `3px solid ${color}` }}
                  >
                    <td className="py-3 font-semibold pl-3" style={{ color: C.text }}>
                      {b.id}
                    </td>
                    <td style={{ color: C.text }}>{b.product}</td>
                    <td style={{ color: C.text2 }}>{store?.name}</td>
          ...
          <ResponsiveContainer width="100%" height={240}>
            <PieChart>
              <Pie data={byCat} dataKey="value" nameKey="name" innerRadius={50} outerRadius={90}>
                {byCat.map((_, i) => (
                  <Cell key={i} fill={PCOLORS[i % PCOLORS.length]} />
                ))}
              </Pie>
              <Legend wrapperStyle={{ fontSize: 11 }} />
            </PieChart>
              ...
              {incidents.map((w, i) => {
                ...
                return (
                  <tr key={i} className="border-t" style={{ borderColor: C.border }}>
                    <td className="py-3" style={{ color: C.text2 }}>
                      {formatDate(w.date)}
                    </td>
                    ...
                    <td>
                      {batchExists ? (
                        <button
                          onClick={() => dispatch({ type: "OPEN_DRAWER", batchId: w.batch })}
                        ...
                        <span style={{ color: C.muted }}>{w.batch}</span>
                      )}
                    </td>
                    <td style={{ color: C.text2 }}>{store?.name}</td>
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-[13px]" style={{ color: C.text2 }}>
          Operational tasks generated from freshness intelligence
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {cols.map((col) => (
          <Card key={col.key} className="p-4" style={{ borderLeft: `3px solid ${col.color}` }}>
            ...
            <div className="space-y-2">
              {col.list.length === 0 && (
                <div className="text-[12px] py-4 text-center" style={{ color: C.muted }}>
                  No tasks
                </div>
              )}
              {col.list.map((t) => {
                ...
                return (
                  <div
                    key={t.id}
                    className={`p-3 rounded-lg ${highlight ? "fp-flash-row" : ""}`}
                    style={{ background: C.bg, opacity: t.status === "done" ? 0.65 : 1 }}
                  >
                    <div className="flex items-center justify-between mb-1">
                      <Pill bg={`${col.color}1A`} color={col.color}>
                        {t.priority}
                      </Pill>
                      <div className="flex gap-1">
                        {isOverdue && (
                          <Pill bg={`${C.red}1A`} color={C.red}>
                            OVERDUE
                          </Pill>
                        )}
                    ...
                    <div className="text-[13px] font-semibold mt-1" style={{ color: C.text }}>
                      {t.type} — {t.product}
                    </div>
                    <div className="text-[12px] mt-1" style={{ color: C.text2 }}>
                      {t.instruction}
                    ...
                    <div className="flex items-center justify-between mt-2">
                      {storeId === "all" && store && (
                        <button
                          onClick={() => {
                            dispatch({ type: "OPEN_STORE_PROFILE", storeId: store.id });
                          }}
                    ...
                    {t.status === "todo" && (
                      <button
                        onClick={() =>
                          dispatch({ type: "UPDATE_TASK", id: t.id, status: "inprogress" })
                        }
                    ...
                    {t.status === "inprogress" && (
                      <button
                        onClick={() => dispatch({ type: "UPDATE_TASK", id: t.id, status: "done" })}
                        className="mt-2 w-full text-[12px] py-1.5 rounded"
...
function ImpactPage({ state, dispatch }: PageProps) {
  ...
  const completed = state.tasks.filter(
    (t) => t.status === "done" && (storeId === "all" || t.storeId === storeId),
  );
  const savings = completed.reduce(
    (sum, t) => sum + (t.type === "Markdown" ? 120 : t.type === "FEFO Rotation" ? 80 : 60),
    0,
  );
  const wasteKg = completed.filter((t) => t.type === "FEFO Rotation").length * 2;
  const co2 = wasteKg * 0.002;

  const league = state.stores
    .filter((s) => s.id !== "DC")
    .map((s, i) => ({
      store: s,
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between no-print">
        <div>
          ...
          <p className="text-[13px]" style={{ color: C.text2 }}>
            Sustainability outcomes from interventions
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <Btn
            ...
            onClick={() => {
              if (navigator.share)
                navigator
                  .share({ title: "Freshness Passport — Impact Report", url: location.href })
                  .catch(() => {});
              else {
                navigator.clipboard?.writeText(location.href);
                dispatch({
                  type: "PUSH_TOAST",
                  toast: { id: uid(), kind: "info", text: "Link copied to clipboard" },
                });
              }
            }}
          >
      ...
      >
        ...
        <div className="text-[22px] font-bold mt-1">
          {activeStore ? activeStore.name : "All Stores Network"}
        </div>
        <div className="text-[14px] mt-1 opacity-90">
          Verified Waste Reduction: {formatPct((wasteKg / 100 || 1) * 12)}
        </div>
        <div className="grid grid-cols-3 gap-4 mt-4">
          <div>
            <div className="text-[11px] opacity-70 uppercase">Period</div>
            <div className="text-[14px] font-semibold">{formatDate(Date.now())}</div>
          </div>
          <div>
            <div className="text-[11px] opacity-70 uppercase">Interventions</div>
            <div className="text-[14px] font-semibold">{completed.length}</div>
          </div>
          <div>
            <div className="text-[11px] opacity-70 uppercase">CO₂ Avoided</div>
            <div className="text-[14px] font-semibold">{co2.toFixed(3)}t</div>
          </div>
        </div>
        <div className="print-only hidden mt-4 text-[11px]">
          Generated by Freshness Passport · {formatDate(Date.now())}
        </div>
      </Card>
            ...
            {league.map((row, i) => {
              ...
              return (
                <tr
                  key={row.store.id}
                  className="border-t"
                  style={{ borderColor: C.border, background: isSelected ? C.light : undefined }}
                >
                  <td className="py-3" style={{ color: C.text }}>
                    {i + 1}
                    {i === 0 && " 🏆"}
                  </td>
                  <td>
                    <button
                      onClick={() =>
                        dispatch({ type: "OPEN_STORE_PROFILE", storeId: row.store.id })
                      }
                    ...
                    {row.store.isNew && (
                      <Pill bg={C.badgeBlueBg} color={C.badgeBlueText} className="ml-2">
                        New
                      </Pill>
                    )}
                  </td>
                  ...
                  <td>
                    <svg width="80" height="20">
                      <polyline
                        points={row.trend.map((v, i) => `${i * 12},${20 - v * 0.4}`).join(" ")}
                        fill="none"
...
function StoreProfilePage({ state, dispatch }: PageProps) {
  ...
  const waste = WASTE_INCIDENTS.filter(
    (w) => w.storeId === store.id && w.date >= n - 7 * DAY,
  ).reduce((s, w) => s + w.cost, 0);
  const freshness = batches.length
    ? batches.reduce((s, b) => s + computeRSL(b, n).score, 0) / batches.length
    : 0;
    ...
    <div className="space-y-5">
        ...
        <div className="flex items-start gap-5">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => dispatch({ type: "TOGGLE_TIP", key: "storeProfile" })}
                className="w-5 h-5 rounded-full text-[11px] font-bold flex items-center justify-center"
                style={{ background: C.light, color: C.accent }}
              >
                ?
            ...
            <div className="flex items-center gap-2 mt-1">
              <Pill bg={C.bg} color={C.text2}>
                {store.code}
              </Pill>
              <Pill bg={C.light} color={C.accent}>
                {store.tier}
              </Pill>
              <span className="text-[12px]" style={{ color: C.muted }}>
                Active since {formatDate(store.activeSince)}
              </span>
            </div>
            <div className="text-[13px] mt-2" style={{ color: C.text2 }}>
              {store.address} · {store.city} · {store.postcode}
            </div>
            <div className="text-[13px]" style={{ color: C.text2 }}>
              👤 {store.manager} · {store.email}
            </div>
          </div>
          <div className="flex gap-2">
            ...
            <Btn
              kind="primary"
              onClick={() => dispatch({ type: "OPEN_PASSPORT_MODAL", prefilledStoreId: store.id })}
            >
              + New Passport
            </Btn>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KpiCard
          ...
          accent={C.accent}
          onClick={() => {
            dispatch({ type: "SET_STORE_FILTER", storeId: store.id });
            dispatch({ type: "NAVIGATE", page: "passports" });
        ...
        <KpiCard
          ...
          onClick={() => {
            dispatch({ type: "SET_STORE_FILTER", storeId: store.id });
            dispatch({ type: "NAVIGATE", page: "rsl" });
            dispatch({ type: "BACK_FROM_PROFILE" });
          }}
        />
        <KpiCard
          ...
          onClick={() => {
            dispatch({ type: "SET_STORE_FILTER", storeId: store.id });
            dispatch({ type: "NAVIGATE", page: "analytics" });
            dispatch({ type: "BACK_FROM_PROFILE" });
          }}
        />
        <KpiCard
                ...
                {state.activity
                  ...
                  .map((a) => (
                    <div
                      ...
                      style={{ color: C.text2 }}
                    >
                      <span>{ACT_ICON[a.type].icon}</span>
                      <span className="flex-1">{a.text}</span>
                      <span style={{ color: C.muted, fontSize: 11 }}>{timeAgo(a.ts)}</span>
                    </div>
                  ))}
              </div>
              <button
        ...
        {tab === "products" &&
          (batches.length === 0 ? (
            <div className="py-12 text-center">
              ...
              <button
                onClick={() =>
                  dispatch({ type: "OPEN_PASSPORT_MODAL", prefilledStoreId: store.id })
                }
                ...
                {batches.map((b) => {
                  ...
                  return (
                    <tr key={b.id} className="border-t" style={{ borderColor: C.border }}>
                      ...
                      <td>{b.product}</td>
                      <td style={{ color: freshnessColor(r.score) }}>{r.rslDays}d</td>
                      <td>
                        <StatusBadge score={r.score} />
                      </td>
                    </tr>
  ...
  return (
    <>
      ...
      >
          ...
          {NAV.map((n) => {
            ...
            return (
              <button
                key={n.key}
                onClick={() => {
                  dispatch({ type: "NAVIGATE", page: n.key });
        ...
        <div className="p-3 text-[11px]" style={{ color: C.muted }}>
          © Freshness Passport 2026
        </div>
      </aside>
    </>
  );
...
function Header({ state, dispatch }: PageProps) {
  ...
  useEffect(() => {
    const h = (e: MouseEvent) => {
      if (state.storeDropdownOpen && ddRef.current && !ddRef.current.contains(e.target as Node))
        dispatch({ type: "TOGGLE_STORE_DD", open: false });
      if (state.bellOpen && bellRef.current && !bellRef.current.contains(e.target as Node))
  ...
  return (
    ...
    >
      ...
      <div className="flex-1" />
      <button
        onClick={() => dispatch({ type: "ADVANCE_TIME", days: 1 })}
      ...
      >
        ⏩ Simulate +1 Day {state.timeOffset > 0 && `(+${state.timeOffset / DAY}d)`}
      </button>
      {state.timeOffset > 0 && (
        <button
      ...
      <div ref={ddRef} className="relative">
        ...
        {state.storeDropdownOpen && (
          ...
          >
            <button
              onClick={() => dispatch({ type: "SET_STORE_FILTER", storeId: "all" })}
            ...
            {state.stores.map((s) => (
              ...
              >
                <button
                  onClick={() => dispatch({ type: "SET_STORE_FILTER", storeId: s.id })}
                ...
                <button
                  onClick={() => dispatch({ type: "OPEN_STORE_PROFILE", storeId: s.id })}
        ...
        {state.bellOpen && (
          ...
          >
            <div
              ...
              style={{ borderColor: C.border }}
            >
              <span className="text-[12px] font-bold uppercase" style={{ color: C.muted }}>
                Notifications
              </span>
              <button
            ...
            {notifs.length === 0 && (
              <div className="p-4 text-center text-[13px]" style={{ color: C.muted }}>
                No notifications
              </div>
            )}
            {notifs.map((nf) => {
              const color =
                nf.kind === "expiry" ? C.red : nf.kind === "overdue" ? C.amber : C.accent;
              return (
                <button
                  key={nf.id}
                  onClick={() => {
                    if (nf.batchId) dispatch({ type: "OPEN_DRAWER", batchId: nf.batchId });
                      ...
                      dispatch({ type: "NAVIGATE", page: "actions" });
                      dispatch({ type: "HIGHLIGHT_TASK", id: nf.taskId });
                    }
                    dispatch({ type: "TOGGLE_BELL", open: false });
                  }}
                  className="w-full text-left px-3 py-2 border-b text-[13px] hover:bg-slate-50"
                  style={{ borderColor: C.border, borderLeft: `3px solid ${color}`, color: C.text }}
                >
                  {nf.text}
                </button>
              );
  ...
  return (
    ...
    >
      ...
      {drawerBatch && (
        <>
          <span>›</span>
          <span style={{ color: C.text }}>Passport: {drawerBatch.id}</span>
        </>
      )}
    </div>
  );
...
function Toasts({ state, dispatch }: PageProps) {
  useEffect(() => {
    const timers = state.toasts.map((t) =>
      setTimeout(() => dispatch({ type: "DISMISS_TOAST", id: t.id }), 3500),
    );
      ...
      {state.toasts.map((t) => {
        ...
        return (
          <div
            ...
            style={{ borderLeft: `4px solid ${color}`, color: C.text }}
          >
            <span>{t.text}</span>
            <button
...
export default function FreshnessPassport() {
  ...
  const [state, dispatch] = useReducer(reducer, initialState, (init) => {
    ...
    const exists = init.stores.some(
      (s) => s.name.toLowerCase() === session.storeName!.toLowerCase(),
    );
  ...
  const renderPage = () => {
    ...
    switch (state.activePage) {
      ...
      case "passports":
        return <PassportsPage state={state} dispatch={dispatch} />;
      case "rsl":
        return <RSLPage state={state} dispatch={dispatch} />;
      case "analytics":
        return <WastePage state={state} dispatch={dispatch} />;
      case "actions":
        return <ActionsPage state={state} dispatch={dispatch} />;
      case "impact":
        return <ImpactPage state={state} dispatch={dispatch} />;
      default:
        return null;
    }
  };
  ...
  return (
    <div
      ...
      style={{ background: C.bg, fontFamily: FONT, color: C.text }}
    >
      <style>{`
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 p-4 md:p-8 max-w-[1400px] w-full mx-auto">{renderPage()}</main>
      </div>
